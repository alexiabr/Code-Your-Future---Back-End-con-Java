// esto es para mostrar que cuando se da como argumento una variable por valor, esto signiica que solo se altera dentro del subproceso, pero
// sigue valiendo lo mismo, mientras que cuando se pasa por referencia, el valor de la variable si es alterado por el subproceso y por ende
// luego de ejecutarlo la variable contiene otro valor


Algoritmo valorVSreferencia
	Definir num Como Entero
	num = 2
	Escribir num
	Escribir "Ahora enviamos el número a la función por valor y el resultado es:"
	elevarAlCuadradoPorValor(num)
	Escribir num
	Escribir "***********"
	Escribir "Ahora enviamos el número a la función por referencia y el resultado es:"
	elevarAlCuadradoPorReferencia(num)
	Escribir num
FinAlgoritmo



SubProceso elevarAlCuadradoPorValor(num Por Valor)
	num = num * num
FinSubProceso

SubProceso elevarAlCuadradoPorReferencia(num Por Referencia)
	num = num * num
FinSubProceso







